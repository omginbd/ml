...
function trainingEval(data, learner, verbose) {
    ...
    const features = new Matrix_1.default(data, 0, 0, data.rows(), data.cols() - 1, data.relationName() + ' features');
    // Copy the last column in the ARFF data into a labels matrix.
    const labels = new Matrix_1.default(data, 0, data.cols() - 1, data.rows(), 1, data.relationName() + ' labels');
...
function staticEval(data, learner, testFileName, normalize, verbose) {
    ...
    const testFeatures = new Matrix_1.default(testData, 0, 0, testData.rows(), testData.cols() - 1, testData.relationName() + ' testFeatures');
    const testLabels = new Matrix_1.default(testData, 0, testData.cols() - 1, testData.rows(), 1, testData.relationName + ' testLabels');
    const confusion = new Matrix_1.default();
...
function randomEval(data, learner, evalParameter, verbose) {
    ...
    const trainFeatures = new Matrix_1.default(data, 0, 0, trainSize, data.cols() - 1, data.relationName() + ' trainFeatures');
    const trainLabels = new Matrix_1.default(data, 0, data.cols() - 1, trainSize, 1, data.relationName() + ' trainLabels');
    const testFeatures = new Matrix_1.default(data, trainSize, 0, data.rows() - trainSize, data.cols() - 1, data.relationName() + ' testFeatures');
    const testLabels = new Matrix_1.default(data, trainSize, data.cols() - 1, data.rows() - trainSize, 1, data.relationName() + ' testLabels');
    const startTime = Date.now();
...
function crossEval(data, learner, evalParameter) {
    ...
    for (let j = 0; j < reps; j++) {
        ...
        for (let i = 0; i < folds; i++) {
            ...
            const trainFeatures = new Matrix_1.default(data, 0, 0, begin, data.cols() - 1, data.relationName() + 'trainFeatures');
            const trainLabels = new Matrix_1.default(data, 0, data.cols() - 1, begin, 1, data.relationName() + ' trainLabels');
            const testFeatures = new Matrix_1.default(data, begin, 0, end - begin, data.cols() - 1, data.relationName() + ' testFeatures');
            const testLabels = new Matrix_1.default(data, begin, data.cols() - 1, end - begin, 1, data.relationName() + ' testLabels');
